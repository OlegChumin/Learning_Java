...
public class Main { // Объявляем класс Main
    public static void main(String[] args) { // Определяем метод main как точку входа в программу
        javax.swing.SwingUtilities.invokeLater(new Runnable() { // Обеспечиваем, что интерфейс будет запущен в потоке диспетчеризации событий
            public void run() { // Определяем метод run, который будет выполнен в потоке диспетчеризации событий
                new Mp3PlayerGUI(); // Создаем экземпляр нашего графического интерфейса
            }
        });
    }
}
