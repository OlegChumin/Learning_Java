...
public class Lesson020SyncronizedPartII {
    public static void main(String[] args) {
        new WorkerFinal().main();
    }
}
...
class WorkerFinal {
    ...
    public void addToList1() { // 1000 раз добавляет случайное число с задержкой 1 мсек в list1
        synchronized (lock1) {
            try {
                Thread.sleep(1);
    ...
    public void addToList2() { // 1000 раз добавляет случайное число с задержкой 1 мсек в list2
        synchronized (lock2) {
            try {
                Thread.sleep(1);
            } catch (InterruptedException e) {
