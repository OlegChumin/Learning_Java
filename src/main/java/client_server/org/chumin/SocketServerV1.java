package client_server.org.chumin;
...
public class SocketServerV1 {
    ...
    public static void main(String[] args) throws IOException {
        ServerSocket serverSocket = new ServerSocket(8000); // создаём серверный сокет на порту 8000
        serverSocket.accept(); // блокируется до подключения клиента (например, через curl или браузер)
        serverSocket.close(); // после подключения клиентом — закрываем сокет и завершаем программу
    }
